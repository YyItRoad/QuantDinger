...
# @param structure_buffer_atr float 0.2 接管K线高点上方ATR缓冲 range=0.0:3.0:0.1
...
def _initialize_position(context, instrument, position, bars, atr_value):
    ...
    g.state = {
        ...
        "risk": max(1e-12, initial_stop - entry_price),
        "lowest_low": float(bars["low"].iloc[-1]),
        "trail_stop": 0.0,
        "exit_ref": "",
    }
    log(
...
def _log_cycle(context, instrument, position, bars, atr_value, stop_before, stop_after, decision):
    ...
    current_pnl = (entry - close) / entry * leverage * 100.0
    stop_pnl = (entry - stop_after) / entry * leverage * 100.0
    log(
        "1H空单持仓检查"
        ...
        + "｜开仓均价=" + _price(entry)
        + "｜ATR=" + _price(atr_value)
        + "｜接管后最低价=" + _price(g.state["lowest_low"])
...
def _exit_pending(position):
    ...
    if status in ("rejected", "failed", "cancelled", "canceled", "expired"):
        log("空单平仓指令失败｜状态=" + status + "｜下一根K线重新判断")
        g.state["exit_ref"] = ""
        return False
    log("等待空单平仓成交｜状态=" + status + "｜剩余数量=" + _price(abs(float(position.amount or 0.0))))
    return True
...
def handle_data(context, data):
    ...
    if len(bars) < atr_period + 1:
        log("空单持仓检查跳过｜品种=" + instrument + "｜原因=ATR历史K线不足｜现有=" + str(len(bars)))
        return
    # 该策略实例已由 direction_mode=short_only 限定为空头，运行器会把
    ...
    if abs(float(position.amount or 0.0)) <= 1e-12:
        if g.state is not None:
            ...
            log("空单仓位已归零，结束管理｜品种=" + instrument + "｜原因=" + str(reason or "外部平仓或已成交"))
            g.state = None
        return
    atr_value = _atr(bars, atr_period)
    if atr_value <= 0:
        log("空单持仓检查跳过｜品种=" + instrument + "｜原因=ATR无效")
        return
    if g.state is None:
    ...
    if float(bars["high"].iloc[-1]) >= stop_before:
        _log_cycle(context, instrument, position, bars, atr_value, stop_before, stop_before, "最高价触及止损，提交空单平仓")
        g.state["exit_ref"] = order_target_value(
            instrument,
            0.0,
            position_side="short",
            reason="atr_short_position_stop",
        )
        return

    g.state["lowest_low"] = min(float(g.state["lowest_low"]), float(bars["low"].iloc[-1]))
    activation = float(g.state["entry_price"]) - _number(context, "trail_activation_r", 2.0, 0.5) * float(g.state["risk"])
    if float(g.state["lowest_low"]) <= activation:
        candidate = min(
            float(g.state["initial_stop"]),
            float(g.state["lowest_low"]) + _number(context, "trail_atr_mult", 3.5, 0.5) * atr_value,
        )
        previous = float(g.state["trail_stop"] or 0.0)
        g.state["trail_stop"] = candidate if previous <= 0 else min(previous, candidate)
    stop_after = _active_stop()
    decision = "继续持有"
    if stop_after < stop_before:
        decision = "继续持有，空单止损下移"
